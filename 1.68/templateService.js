import{bX as _,a8 as o,b0 as c}from"./index.js";async function q(r){const{data:e,error:t}=await o.from("templates").select("*").eq("id",r).eq("is_delete",!1).single();if(t)throw t;return e}async function h(r){const{data:e,error:t}=await o.from("templates").select("*").eq("parent_id",r).eq("is_delete",!1).order("sort");if(t)throw t;return e||[]}async function m(r,e){let t=o.from("process_print_areas").select(`
            id,
            title,
        ...
        `).eq("template_id",r).single();e&&(t=t.abortSignal(e));const{data:a,error:s}=await t;if(s)throw s;return a}async function A(r){const{data:e,error:t}=await o.from("process_print_areas").select("*").eq("object_id",r).eq("is_delete",!1).single();return t?(console.error("인쇄영역 조회 오류:",t),null):e}export{g as createTemplateReadContext,h as getChildTemplates,y as getParentTemplates,A as getPrintAreaByObjectId,q as getTemplate,w as getTemplateProduct,R as getTemplateProductForRead,m as getTemplatesByObjectId,T as getTemplatesByObjectIdForRead};
