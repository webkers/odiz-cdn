import{bR as _,a7 as o,a_ as c}from"./index.js";async function b(r){const{data:e,error:t}=await o.from("templates").select("*").eq("id",r).eq("is_delete",!1).single();if(t)throw t;return e}async function h(r){const{data:e,error:t}=await o.from("templates").select("*").eq("parent_id",r).eq("is_delete",!1).order("sort");if(t)throw t;return e||[]}async function m(r,e){let t=o.from("process_print_areas").select(`
            id,
            title,
